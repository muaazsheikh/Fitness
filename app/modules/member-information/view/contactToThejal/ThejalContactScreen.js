...
const ThejalContactScreen = ({ navigation }) => {
  ...
  async function fetchMyAPI() {
    try {
      setIsLoading(true);
      const response = await getThejalContact();
      setIsLoading(false);
  ...
  const handleCreateContact = async () => {
    ...
    try {
      ...
      const response = await createThejalContactApi(requestParam);
      setIsLoading(false);
      clearEdit();
      fetchMyAPI();
    } catch (error) {
  ...
  const handleDelete = async (item) => {
    setIsLoading(true);
    try {
      const response = await createThejalDeleteApi(item.id);
  ...
  const handleEdit = async () => {
    ...
    const requestParam = {
      title: titleInput,
      content: contentInput,
      imgs: pickedImageList || null,
    };
    try {
      const response = await createThejalEditApi(
  ...
  const CreateButton = () => (
    <View style={styles.buttonView}>
      <PrimaryButton
        ...
        edit={true}
        color={true}
        onPress={() => (isEdit ? clearEdit() : handleNavigation(""))}
      />
      <PrimaryButton
        title="Save"
        style={90}
  ...
  const ContactCard = ({ item }) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate(NavigationStrings.THEJAL_CONTACT_DETAIL, item);
        }}
      >
...
export default ThejalContactScreen;
