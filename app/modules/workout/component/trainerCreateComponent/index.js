...
import trainerShareMemberApi from "../../../../api/trainer-workout/trainerShareMember";
...
const Card = ({ item, navigation, color, onChange }) => {
  ...
  const handleNavigation = async (nav, id) => {
    try {
      onChange();
      const response = await trainerShareMemberApi(id);
    } catch (error) {
  ...
  return (
    ...
    >
      <View style={styles.cardView}>
        <View>
          <LogCreated
            ...
            color={color}
            traineConfirm={item?.workoutLogSummary?.traineeConfirm}
      ...
      <View style={{ alignSelf: "flex-end", marginVertical: 10 }}>
        {!item?.workoutLogSummary?.sharedToMember ? (
          <PrimaryButton
            title={"Share to Member"}
            style={120}
            color={true}
            onPress={() =>
...
const TrainerCreateComponent = ({ navigation, data, color, onChange }) => {
